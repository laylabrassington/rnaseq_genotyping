...
in_bam=/filepath/$bamID.Aligned.sortedByCoord.out.bam 
...
sampleID=$(sed -n ${SLURM_ARRAY_TASK_ID}p /filepath/id_list)

# print out the ids 
echo "bam ID: $bamID"
echo "sample ID: $sampleID"

in_bam_auto=/out_directory_filepath/$sampleID.auto.bam
out_bam_sorted=/out_directory_filepath/$sampleID.sorted.bam
out_bam_duplicates=/out_directory_filepath/$sampleID.dup.bam
out_txt_dupmetrics=/out_directory_filepath/$sampleID.dup.txt
out_bam_cigar=/out_directory_filepath/$sampleID.cigar.bam
out_bam_readgroups=/out_directory_filepath/$sampleID.reg.bam
out_table_recalibr=/out_directory_filepath/$sampleID.rbqs.table
out_bam_recalibrat=/out_directory_filepath/$sampleID.rbqs.bam
out_g_vcf=/out_directory_filepath/$sampleID.g.vcf
out_g_vcf_filt=/out_directory_filepath/$sampleID.filt.vcf
...
mv /out_directory_filepath/$sampleID.filt.vcf.gz /out_directory_filepath/out_vcfs
...
wc -l $out_g_vcf
gzip -cd /out_directory_filepath/out_vcfs/$sampleID.filt.vcf.gz | grep 'PASS' | wc -l

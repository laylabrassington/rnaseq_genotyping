...
```{r}
metadata <- read.delim('/filepath/metadata.txt',sep = '\t')
lid_altid_map <- metadata %>% dplyr::select(LID, alt_id)
#write.table(lid_altid_map,'/filepath/lid_altid_map.txt',row.names=F,sep='\t',quote=F)

#then ran sbatch script: /filepath/moving_files
```
...
```{r}
#ran /filepath/processing_rnaseq_genotype_data_10feb25
```
...
```{r}
king_kin0 <- fread("/filepath/king_kinship_19feb25.kin0")
```
...
```{r}
king_kin0 <- fread("/filepath/king_related_19feb25.kin0")
```
...
```{r}
eigenvec <- read.table("/filepath/plink_pca_19feb25.eigenvec", header = FALSE)

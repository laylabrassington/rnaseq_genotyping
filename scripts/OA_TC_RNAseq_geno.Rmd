...
need to move files around in cluster and rename 
```{r}
metadata <- read.delim('/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/qc/metadata_pca_5feb25.txt',sep = '\t')
lid_altid_map <- metadata %>% dplyr::select(LID, alt_id)
#write.table(lid_altid_map,'/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/lid_altid_map.txt',row.names=F,sep='\t',quote=F)

#then ran sbatch script: /nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/moving_files
```

needed to add these names 
```{r}
# LID0318 OA_tc_rnaseq_18jul24VANP_LID0318_1_UN_Whole_C1_NUDRD_A05145_22CTFYLT4_GTGTGGAG
# LID0645 OA_tc_rnaseq_18jul24VANP_LID0645_1_UN_Whole_C1_NUDRD_A05175_22CTFYLT4_TGGAGATT
# LID0653 OA_tc_rnaseq_18jul24VANP_LID0653_1_UN_Whole_C1_NUDRD_A05181_22CTFYLT4_GTTGAAGG 
# LID0660 OA_tc_rnaseq_18jul24VANP_LID0660_1_UN_Whole_C1_NUDRD_A05187_22CTFYLT4_ACGACGTC 
# LID2698 12471-AL-0005_11
# LID2452 12471-AL-0006_03
# LID0653 12471-AL-0006_33 
# LID0318 12471-AL-0006_34 
# LID2547 12471-AL-0007_21
# LID0239 OA_tc_rnaseq_26aug2412070-AL-0001_1_S1
```
...
then need to merge vcf files 
```{r}
#ran /nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/geno/out_vcfs/comb/processing_rnaseq_genotype_data_10feb25
```


okay seems like even though i changed all the filenames it still ran everything else with the old names 
...
```{r}
king_kin0 <- fread("/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/geno/out_vcfs/comb/out/king_kinship_19feb25.kin0")
```
...
```{r}
lid_altid_map=read.delim("/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/lid_altid_map.txt",sep = '')
...
```{r}
dat=read.delim('/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/meta_12feb25.txt',sep = '\t')
...
dat=read.delim('/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/meta_12feb25.txt',sep = '\t')
dat %>% filter(tid %in% c("1499", "1491")) %>% select(LID, tid, rid, interview_location_med, med_date, sex_medical)
kinship_df_filtered %>% filter(tid_ID1 %in% c("1499", "1491")) %>% filter(Relatedness>0.3)
kinship_df_filtered %>% filter(tid_ID1 %in% c("1499")) %>% filter(tid_ID2 %in% c("1499"))
```


these seem to be fine (besides 1499) 
```{r}
kinship_df_filtered %>% filter(expected=="yes") %>% filter(Relatedness<0.3)
kinship_df_filtered %>% filter(tid_ID1=="944") %>% filter(Relatedness>0.3)
kinship_df_filtered %>% filter(tid_ID1=="745") %>% filter(Relatedness>0.3)
kinship_df_filtered %>% filter(tid_ID1=="955") %>% filter(Relatedness>0.3)
kinship_df_filtered %>% filter(tid_ID1 %in% c("944", "745", "955")) %>% ggplot(aes(x = tid_ID1, y = Relatedness, color = expected)) + geom_point(alpha=0.25) + theme_classic() + 
  scale_color_manual(values = c("yes" = "purple", "no" = "orange")) + 
  labs(title = "Kinship", x = "Sample ID", y = "Kinship Value", color = "Self vs Other")
kinship_df_filtered %>% filter(tid_ID1 %in% c("944", "745", "955")) %>% ggplot(aes(x = Relatedness, fill = expected)) + geom_density(alpha = 0.5) + theme_classic() + labs(title = "Density Plot of Kinship", x = "Kinship Value", y = "Density", fill = "Self vs Others") + scale_fill_manual(values = c("yes" = "purple", "no" = "orange")) + theme(axis.text.x = element_blank())
```
...
```{r}
king_kin0 <- fread("/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/geno/out_vcfs/comb/out/king_related_19feb25.kin0")
```
...
```{r}
eigenvec <- read.table("/nobackup/lea_lab/layla/OA_tc_rnaseq_3dec24/geno/out_vcfs/comb/out/plink_pca_19feb25.eigenvec", header = FALSE)
